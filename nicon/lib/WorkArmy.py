...
class WorkArmy(QThread):    
    ...
    def fn_param(self ,  macros , excels ):
        ''' 3차원 데이터 셋 '''
        self.__core         = core.Core()
        self.__type         = 'A' # 일괄 하나의 작업을 통으로 처리
    ...
    def fnParamMany(self , macros , excels):
        ''' 3차원 데이터 셋 '''
        self.__core         = core.Core()
        self.__type         = 'B' # 개별 행 단위로 작업을 하나씩 처리.
        self.__macros       = macros
        self.__excels       = excels
    ...
    def runTypeA(self):
        ''' 일괄 수행 asis '''
        try:            
            for z in range(0 , len(self.__macros)):
                self.__mk_excels    = self.__excels[z]
                self.__mk_datas     = self.__macros[z]
    ...
    def runTypeB(self): 
        ...
        try:            
                        ...
                        if self.__power == True:
                            ...
                            for i in self.__macros[y]:                                                                                           
                                ...
                                if ( step_name == '끝') :                                    
                                    if self.__pointRow[y] == 0:
                                        ...
                                        if y == (len(self.__pointRow)-1):
                                            try:
                                                ...
                                                #self.__dbconn.macro_job_log( file_nms ,'S') # db 로그 생성
                                            except Exception as e:
                                    ...
                                    print('구간반복 - 종료','='*15)
                                elif (step_name in ['클릭','붙여넣기','글씨쓰기','선택하기','중복선택','랜덤선택','지정선택','방향전환','무시','캡쳐','D&D','랜덤대기','입력대기','자방[숫자]','자방[문자]','자방[혼합]','브라우저'] ) :
                                    if (self.__power == True):
                                        self.__core.fnMain( step_name , i , _j ) 
            ...
            try:            
                w2ji.sendTelegramMsg( f'개별 : [{file_nms}]\n 전체소요시간 [{_hour}:{_minute}:{_second}] ]\n 예상시간 [{pt}] \n ===== 완료 ====='  )   
                #self.__dbconn.macro_job_log( file_nms ,'E') # db 로그 생성                         
            except Exception as e:
