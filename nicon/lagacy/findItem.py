...
'''
구글 드라이버로 키워드 검색하여 필요한 자원을 크롤링한다.
'''
...
class Search():
    ...
    def text2img( self , base64_data:str ):
        ...
        try:
            ...
            #img.show()
            sharpened_img = self.upscale_and_sharpen(img, scale_factor=2) #확대보정               
    ...
    def get_google_images_no_api(self , keyword , target_keywords , except_word):
        ...
        with Chrome(options=options) as driver:             
            ...
            #eles = driver.find_elements(By.CSS_SELECTOR, '[jsname="dTDiAc"]')
        ...
        totproc = len(results) # 전체 처리 건수 추출
        print('\n')
        ...
        print(f"검색어[{keyword}] 처리 [총:{totproc}, 성공:{totsucc}, 실패:{totfail}].")
...
if __name__ == "__main__":   
    ...
    while(True):
        try:
            ...
            _tot_proc = 0 # 전체 처리 건수
            _tot_succ = 0 # 신규 집계 건수
            _tot_fail = 0 # 미대상 건수


            for word in tqdm(keywords_list , desc='\n검색중.......'):                
                _sub_cnt += 1 # 검색 횟수 가산
                __proc , __succ , __fail = ss.get_google_images_no_api( word , target_keywords , except_word )
                _tot_proc += __proc
                _tot_succ += __succ
                _tot_fail += __fail
            ...
            w2ji.send_telegram_message(  f'[{_tot_cnt}]회차 - 데이터 정상 수집완료 \n [전체:{_tot_proc}, 신규:{_tot_succ}, 미대상:{_tot_fail}]' )
        ...
        print(f"{datetime.now(timezone('Asia/Seoul')).strftime('%Y-%m-%d %H:%M:%S')} ⏳ 다음 수집까지 24시간 대기합니다...")
        time.sleep(3600*24)
